...
| 도메인 충돌(같은 테이블/파일/엔티티) | **순차 그룹핑** — §도메인 분류(B-1)로 판정된 그룹만 순차, 나머지는 병렬 유지. **판정 SSoT = `shared/scripts/fr-lanes.py`**(같은 레인=직렬), 테이블 충돌은 `table:{이름}` 의사파일로 주입. 판정 불능·스크립트 실패 = **무조건 순차** |
