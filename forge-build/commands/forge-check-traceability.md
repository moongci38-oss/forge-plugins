...
traceability gap이 구현미달(코드결함) vs 스펙노후 중 무엇인지 모호할 때 `advisor-strategist` 조언을 구한다(모델 = `advisor-model-resolve.sh` 출력, 기본 Fable 5.1). **advisory-only — 게이트 차단 아님. 미가용·실패 시 기본 흐름 진행(fail-open).**
