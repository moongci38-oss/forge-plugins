...
로그인 경로를 **추측하지 않는다** — 소스에서 실제 라우트를 찾아낸다.
하드코딩한 `/api/login` 가정이 틀리면 이후 전 단계가 조용히 어긋난다.

> **이 단계를 실행할 때 Read**: `reference.md §2. 로그인 엔드포인트 발견 (FIX-2)`
> — 스택별 탐색 명령·후보 우선순위·발견 실패 시 폴백이 거기 있다.
...
`qa-config.json` 의 `servers[]` 를 순회해 QA 전용으로 띄우고, 끝나면 `restore-dev` 정책으로
원래 dev 프로세스를 best-effort 복원한다(**완전 보장은 아니다**). 포트 충돌의 실제 판별·조치는
여기 QA-PORT 처리가 맡는다 — Step 1.6 은 서버 기동 전이라 경고까지만 한다.

> **이 단계를 실행할 때 Read**: `reference.md §5. 개발 서버 생명주기`
> — 기동·헬스체크·정지·복원 명령 전문과 split-server 분기가 거기 있다.
...
**순서가 핵심이다**: auth 계정 seed 를 admin 의존 seed 보다 **반드시 먼저** 넣는다.
뒤집으면 admin 을 만들려는데 admin 이 있어야 하는 교착이 난다. QA 데이터는 dev 와 격리한다.

> **이 단계를 실행할 때 Read**: `reference.md §8. DB Seed + 격리`
> — seed 순서·격리 방식·스택별 명령이 거기 있다.
...
> 출처 우선순위 표(Spec FR/legacy/Human/금지) + 기대값 출처 명시 표 + scenarios.md 형식 예시 → `reference.md §시나리오 출처 격리` (필요 시 Read)
...
> Coverage Map 알고리즘(entity×action×screen×viewport python 개념코드) + flow-chain/round-trip/entity-CRUD/responsive/value-axis 상세 검증 스크립트·표 → `reference.md §Coverage Map 검증 상세` (필요 시 Read)
