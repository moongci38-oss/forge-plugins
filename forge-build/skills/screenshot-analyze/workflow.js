...
- model = "${codexModel}" (Vision 레그 tier — codex:high)
...
Codex 응답(JSON) 파싱 → StructuredOutput(description/elements/issues/verdict).`,
    ...
    log(`Codex(sol) Vision: ${result?.verdict || 'done'}`)
  } catch (e) {
    log(`Codex(sol) Vision 실패 → Claude 자체 분석 fallback`)
  }
} else {
