...
- [ ] 의존성 취약점 없음
- [ ] CORS/CSP 설정 적절
- [ ] **실행 기반 보안 검증** — 전용 scorer(`forge-check-security-exec`)는 미사용으로 2026-08-11
      제거됐다. `/forge-check-security` 의 익스플로잇 패턴 검사(S14)가 정적으로 근접 커버하나
      실제 실행 판정은 아니다. 인증·권한·외부입력을 다루는 변경이면 `/forge-check-security` 필수.
