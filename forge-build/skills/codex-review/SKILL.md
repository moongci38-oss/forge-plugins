...
> **인터페이스 구분(harness #3 2026-07-30)**: 이 SKILL = **파이프라인 인터페이스**(P3~P7 Forge Dev 단계). 수동 `/codex-review` 슬래시는 `commands/codex-review.md`.
> ⚠️ 2026-09-17 정정 — 구 표기 "P3~P7 Forge Dev 단계 **자동 호출**" 폐기. 자동 호출은 **기본 off** 다: `.claude/hooks/codex-gate-enforce.sh:117` 이 `[ "${CODEX_REVIEW_AUTO_STAGES:-off}" = "off" ] && exit 0` 으로 미설정=off 처리한다(팀 비용 절감). 재현: `grep -n 'CODEX_REVIEW_AUTO_STAGES:-off' .claude/hooks/codex-gate-enforce.sh`. 의도적 이중 인터페이스 — 한쪽 삭제 금지, stage 분기·`CODEX_REVIEW_AUTO_STAGES` 규약 공유, 로직 변경 시 양쪽 동기.

Claude 1차 리뷰의 동일 모델 맹점 보완용 2차 게이트. SDD·PGE·Forge Dev 모든 단계에서 사용 가능.

## 역할

Claude 1차 리뷰의 동일 모델 맹점을 보완하는 OpenAI Codex(gpt-5.6-sol = registry `codex:high`) 경유 2차 리뷰 게이트.
⚠️ 구 표기 "gpt-6-astra" 는 2026-09-17 폐기 — 사람 지시 "advisor 에서만 최고급 모델 사용해"(최고급 `codex:max` 는 advisor 전용). effort 는 그대로. 대체가 아니라 추가 검증이며 stage(plan/code/test/final/bugfix)별로 차등 blocking을 적용한다.

## 컨텍스트

SDD·PGE·Forge Dev 파이프라인 전 단계에서 호출 가능. Forge Dev 통합 지점은 P3/P4(plan, blocking) / P5 Check P5.7-X(code, 권고) / P6 Check 6-TX(test, 권고) / P7 Check 7-X(final, blocking · effort=xhigh) / 버그 patch 후(bugfix, 수동). `~/forge/.env`의 `CODEX_REVIEW_AUTO_STAGES`로 자동 발동 stage를 제어한다.

## 출력

⛔ **`mcp__codex__codex` 직접 호출로 대신하지 마라** — 산출물·INDEX 기록, 재호출 **cap=1**,
stage별 고정 rubric 이 전부 빠진다. 상세·판별 명령 → `commands/codex-review.md §직접 부르지 마라`
⚠️ **이 레인은 단일 Codex 다** — 2벤더 교차는 `/cr-triple`·`/cr-double`(forge-multi)이고 증거도
`.claude/audit/cr-evidence/` 에 남는다. 두 레인의 증거 경로가 다르므로 **한쪽만 보고 "검수 없음"
으로 판정하지 마라**(2026-09-13 cr-final HIGH 정정).

⚠️ **차단선은 `severity`** — `verdict=FAIL` 이어도 critical/high 0건이면 WARN 으로 강등하고 진행한다
(medium/low 는 권고). **이 강등은 이 레인 한정**이고 forge-multi 는 `combined<60` 기준을 따로 쓴다.
로직 정본 → `commands/codex-review.md §Step 7`. 근거: 2026-09-13 PR #545.
...
단독 호출 = 현행 유지. forge-multi Workflow에 흡수 가능 (mode='double' — Claude+Codex).
실행: `Workflow({ script: Bash("cat ${FORGE_ROOT:-$HOME/forge}/.claude/skills/forge-multi/workflow.js"), args: { targetPath, mode: 'double', stage, repoRoot } })`
...
단축 래퍼: `/forge-plan-review`, `/forge-code-review`, `/forge-test-review`, `/forge-final`, `/forge-bug-review`.
...
상세 정책 (스킵 패턴·다운그레이드·diff 처리): `~/forge/dev/rules-on-demand/codex-review-policy.md`.
...
6. **중복검사 축**: 이번 리뷰의 지적이 **직전 라운드·다른 레그가 이미 낸 지적과 같은 것인지** 먼저 대조하고, 같으면 새 지적으로 세지 말고 `기존 지적 재발(미수정)` 으로 표시한다. ⚠️ **적용 범위 = 프롬프트에 이전 지적이 실제로 제공된 호출만이다** — 현행 단독 호출 경로(`commands/codex-review.md`)는 diff·프롬프트만 넘기므로 그 경우 이 축은 **이번 보고서 안의 지적 간 중복**만 대조하고, 축을 어느 범위로 적용했는지 결과에 1줄 명시한다(안 적으면 안 본 것). 교차 레그·라운드 대조는 이전 지적을 컨텍스트로 넘기는 소비처(forge-multi dedup 단계·재검수 라운드 프롬프트) 몫이다 — 문서가 배선에 없는 입력을 약속하지 않는다(cr-final 2026-08-27 HIGH 정정). 왜: 같은 말을 다시 세면 지적 수가 부풀어 plateau 판정(3회 동일 = 승격)이 오작동하고, 사람은 "리뷰가 일하고 있다"고 착각한다. ⚠️ 무력화되는 입력: 같은 결함을 다른 문장으로 적으면 문자열로는 안 겹친다 — **결함의 위치(파일·함수)로 대조**한다.
