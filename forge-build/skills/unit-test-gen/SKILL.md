...
## --post-phase 모드 (opt-in)

`/unit-test-gen --post-phase` 플래그 사용 시, Phase 완료 후 변경 범위 기반 테스트 분류를 수행한다:

**File Classifier** (git diff 또는 spec 기반):
| 분류 | 판단 기준 | 생성할 테스트 |
|------|----------|------------|
| TDD | 새 함수·클래스 추가 | 단위 테스트 |
| E2E | API 엔드포인트·페이지 변경 | E2E 통합 테스트 |
| Skip | 리팩터링·주석·설정 변경만 | 생성 불필요 |

**진행 규칙**:
1. 분류 결과를 사용자에게 표 형식으로 보고 → 승인 후에만 테스트 생성 시작 (승인 없으면 생성 금지)
2. **bug-discovery-no-fix**: 테스트 작성 중 버그 발견 시 `// FIXME: <설명>` 주석만 추가. 즉시 수정 금지 — 수정은 `/healer` 위임.
3. **E2E blocker no-skip**: E2E 분류된 파일의 테스트는 skip 처리 금지. 환경 미비 시 [STOP] 보고.
