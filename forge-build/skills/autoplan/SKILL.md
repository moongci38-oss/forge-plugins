...
**리뷰어 결측 처리 (E3, 2026-09-07 신설 — 판정선 무변경)**

관점 하나가 응답하지 않았거나(죽음) 스키마를 어겼으면 그 관점은 **"이견 없음"이 아니라 "안 물어봤다"** 이다.
쉽게 말하면 **아무 말도 안 한 사람을 찬성표로 세지 않는다.**

- 그 축의 점수를 상상해서 채우지 않는다. `null` 로 두고 Rubric 표에 **"미측정(사유)"** 으로 적는다
  — 0점("나쁘다")과 결측("모른다")은 다른 것이다.
- `--skip ceo` 처럼 **일부러 뺀 것**은 결측이 아니라 `SKIPPED` 다. 의도와 사고를 같은 칸에 넣지 않는다.
- **3관점이 전멸하면 판정을 만들지 않고 멈춘다**(fail-closed). 종합할 리뷰가 없는데 점수를 내면
  아무도 안 본 기획서에 합격증이 붙는다.
- 가중합 산식·70점 선·Wave 5 ESCALATE 3분법은 **그대로다** — 이 절은 지표(무엇을 기록하나)만 늘린다(E-3).

근거: `workflow.js` 는 `ceoResult?.killSignal` 이 `undefined` 면 그냥 통과시켰다 — 죽은 리뷰어가
조용히 "Kill Signal 없음"으로 집계됐다. 판정 코드·테스트 → `workflow.js §<judge-health:start>` ·
`tests/reviewer-health.test.mjs`.
폐기조건: 리뷰어 스폰 실패가 런타임에서 예외로 올라오게 바뀌면(= 침묵이 불가능해지면) 이 절을 재검토한다.
