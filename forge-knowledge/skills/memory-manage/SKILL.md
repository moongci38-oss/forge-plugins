...
**⚠️ 세기·나이 정렬은 LLM 이 하지 않는다 — 스크립트가 한다**(2026-09-17 LLM→프로그램 분할선
조사 G1). 위 한 줄은 개수만 세고 "최고령 상위 N개"는 여전히 LLM 이 날짜를 눈으로 파싱해
정렬하고 있었다 — 그러면 같은 파일에서도 회차마다 순서가 달라진다. 아래가 둘을 한 번에 낸다
(⛔ 이 스크립트는 **아무것도 지우지 않는다** — surface 전용):

```bash
# 절대경로 필수(CWD 상대경로 금지). 기본 임계값 30 · 상위 5개
python3 "${FORGE_ROOT:-$HOME/forge}/shared/scripts/memory-gc-surface.py" "$MEMORY_PATH"
# rc 0 = 임계값 이하 · rc 1 = 초과(아래 advisory surface 실행) · rc 2 = 판정 불가(파일 없음)
# 출력 JSON: {count, threshold, over, oldest:[{index,heading,date}]}
```

아래 1번(최고령 surface)은 위 출력의 `oldest` 를 **그대로 옮긴다 — 다시 정렬하지 않는다.**
2·3번(유사 주제 병합·세부 이관)은 의미 판단이라 그대로 LLM 몫이다.
코드펜스 오집계·날짜 파싱이 무력화되는 입력은 스크립트 헤더 주석이 정본이다.
