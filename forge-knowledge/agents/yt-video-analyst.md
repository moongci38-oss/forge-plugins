...
4. **실행 가능 항목**: 바로 적용 가능한 행동 목록 (체크박스)
   - 일반적 제안이 아닌, `forge-workspace.json`에 등록된 프로젝트에 구체적으로 적용 가능한 항목
5. **관련성 평가**:
   - 각 등록 프로젝트별: 1-5점 + 이유 (forge-workspace.json 참조)
   - 비즈니스: 1-5점 + 이유
...
**시스템 현황 참조:**
- Business 워크스페이스: Forge (S1→S4), Forge Dev (Phase 1→4)
...
- **Business 워크스페이스**: Forge 기획 파이프라인 (S1→S4), Forge Dev 개발 파이프라인 (Phase 1→4)
