...
### Wave 3 (GitHub PR 모니터링 — P1 추가, 2026-05-08)

매일 아침 develop/main branch의 PR 상태를 체크하고 Telegram 알림.

**실행 시간**: 일일 자동화 (매일 09:00 KST cron)

**작업**:
1. `gh pr list --state open --base develop --json title,number,createdAt,author,reviewDecision` 실행
2. `gh pr list --state open --base main --json title,number,createdAt,author,reviewDecision` 실행
3. 결과 정리: Pending PR 목록 + Approved but not merged 목록 + Blocked 목록
4. Telegram 알림 발송 (forge_agent_server_bot)
...
✅ Merged: N개
🔄 Pending review: N개
⏳ Approved, waiting merge: N개

[각 PR 링크 + 작가 + 생성일]
```

**구현**: `bot.mjs`에 `/loop 24h` 기반 모니터링 또는 별도 Routine 스케줄링
