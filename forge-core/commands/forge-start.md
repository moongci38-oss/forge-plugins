...
- ⚠️ `MSYS2_ARG_CONV_EXCL='*'` 필수 — Git Bash 가 `$HOME/…`·`/mnt/<drive>/…` 인자를 `C:/Program Files/Git/…` 로 자동 변환해 **조용히** 깨뜨린다(2026-08-16 실측: 가드 없이 `wsl -e cat $HOME/…` → No such file). WSL 사용자 홈이 다른 머신도 `$HOME` 참조라 그대로 동작한다.
