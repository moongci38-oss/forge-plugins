...
- **파일을 통째로 Read 하기 전에 `grep -n`·`head`·`sed -n '<범위>p'` 로 필요한 부분부터 연다.** 필요한 게 30줄인데 2,000줄을 담으면 그 세션의 남은 예산을 그만큼 먹는다(쉽게 말하면 **사전을 통째로 외우지 말고 찾아보라**는 것).
- ⚠️ **§Subagent 결과 검증(L-38)과 다른 축이다.** L-38 은 "추측 말고 **실측하라**"(정확성)이고, 이 절은 "실측하되 **좁게 열어라**"(예산)다. 좁게 여는 것이 실측을 건너뛸 근거가 되지는 않는다 — 판정에 필요한 만큼은 반드시 연다.
- 예외: 파일이 작을 때(수십 줄)·구조 전체를 봐야 할 때(리팩터링 대상·전면 재작성)는 그냥 전량 Read 한다.
- 근거: 이 세션의 예산 측정은 `session-context-budget.sh` 가 상시 하고 있고(§컨텍스트 토큰 관리), 압축 임계값에 먼저 닿는 주범이 대용량 Read 반환이다.
- 폐기조건: 도구 응답 offload 훅이 Read 반환까지 자동 축약하게 되면 이 절을 재검토한다.
...
**룰 편집은 그 세션 자신에게 적용되지 않는다 (2026-08-26)**: CLAUDE.md·`rules/` 는 세션 시작 시 1회 로드된다 — 세션 중 고친 규칙은 `/clear`·`/compact`·재시작 전까지 **이 세션은 못 본다**. "방금 고친 규칙을 지금부터 지키겠다"는 선언은 착시다(공식 캐싱 문서 §Editing CLAUDE.md mid-session).
...
- ✅ Agent 도구에 세션 제약은 없다(Human 지시 2026-08-26 — 허용이지 의무 아님, 4축 판단은 그대로) — 구 "AgentTool 금지 세션 예외" 는 폐기. 정본 `forge-core.md §병렬 실행`.
...
- **middle·deep** (다중파일·로직 추적·아키텍처 이해·크로스시스템 종합) → `Agent(model:"sonnet")` subagent
- ⚠️ **검색에 Opus 를 쓰지 않는다**(2026-08-25 Human 지시 — *"sonnet 은 검색용으로 써, haiku 랑 같이"*).
  구 3분법의 `deep → opus` 는 폐기했다. **Opus 는 워커(구현)와 오케스트레이터의 자리**이고
  검색은 반환량이 크고 결론만 쓰이므로 그 tier 를 쓸 이유가 없다(`model-routing.md §워커 tier`).
  폐기조건: Sonnet 검색이 아키텍처 추적에서 반복 실패한 사례가 2건 나오면 재검토한다.
