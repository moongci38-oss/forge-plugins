...
- **하네스 계획서·위임 프롬프트**: `${FORGE_OUTPUTS:-$HOME/forge-outputs}/11-platform/pipelines/plans/` · 리포트는 `${FORGE_OUTPUTS:-$HOME/forge-outputs}/11-platform/pipelines/harness-gaps/`(절대경로 — `../reviews/` 상대표기는 2026-08-05 정정. 구 표기는 사문화된 `pipelines/reviews/`를 가리켜 §하네스 갭 리포트 SSoT와 모순됐다)
...
- ⛔ **Orca ADE = 2026-08-02 사용 중지**(Human 결정 — 지연). 병렬 실행은 위 2분법+Workflow 로 **완결**한다 — Orca 경로를 새로 배선하지 않는다. 중지 방식·복구 명령·재개 규약 → `rules-on-demand/orca-orchestration.md`
