...
> ⚠️ **측정 주의 (H-5 정정, 2026-08-22)**: 레드팀 자산을 `grep -rli 'red.?team|레드팀'` 로
> **내용 검색**하면 0 건이 나온다 — 실제 자산은 `shared/scripts/red-team/` 에 **6건**
> (ASI01·02·06·07·09·10 시나리오, 커밋 `60300cc7`) 있는데 파일 **내용**에 그 단어가 없다.
> **경로도 함께 센다.** 재현:
>   `find "$FORGE_ROOT" -path '*red-team*' -name '*.md' -not -path '*/worktrees/*' | wc -l`  → 6
>   `grep -rliE 'red.?team|레드팀' "$TP" | wc -l`  → (구 측정식 — 내용 검색이라 자산을 못 본다)
>   ⚠️ **`-E` 는 r5 에 붙였다.** 종전 명령에는 없어서 `?`·`|` 가 리터럴로 해석됐다 —
>     즉 이 줄은 자기가 주장하는 값을 재현하지 못하는 명령이었다. 그리고 결과 수치를
>     여기 박지 않는다: 검색 대상이 늘면 바뀌는 관측치라 stale 이 된다. 정본은 명령이다.
> ⚠️ **다만 갭 자체는 남는다**: 시나리오 문서는 있으나 **실행 기록이 0건**이다
> (`forge-outputs` 전역에 red-team 실행 산출물 없음). "자산 없음"이 아니라
> **"자산은 있는데 한 번도 안 돌렸다"** 가 정확한 진단이고, 후자가 더 나쁘다 —
> 만들어 두고 잊은 것이기 때문이다.
> 폐기조건: 레드팀 실행 기록이 분기 1회 이상 남기 시작하면 이 주의를 삭제한다.
