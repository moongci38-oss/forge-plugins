...
| 지표 | 측정 방법 | 기준값 |
|------|---------|:-----:|
| 세션 시작 토큰 | rules + CLAUDE.md + MEMORY.md 합산 — **문자수 ÷ 4** (`wc -m`, `wc -c` 아님) | < 12,000 |

> ⚠️ **단위를 못 박는다 (H-2 정정, 2026-08-22).** 종전 정의가 `wc -c / 3` 이었는데
> 실제 감사들은 `바이트 ÷ 4` 로 재 왔다. 한글은 UTF-8 에서 글자당 3바이트라
> **어느 쪽으로 읽느냐에 따라 같은 파일이 22,538 과 13,527 로 1.7배 갈린다**
> (2026-08-22 실측: 90,907 B / 54,108 chars). 기준값 `< 12,000` 은 하나뿐인데
> 재는 자가 둘이면 PASS/FAIL 이 자의 선택으로 결정된다.
> **문자 기준으로 통일한다** — 바이트는 인코딩에 종속돼 같은 글이 언어마다 다른 값이 된다.
>
> ⚠️ 그리고 **`÷4` 자체가 영어 휴리스틱이다.** 한글은 글자당 토큰 비율이 영어와 달라
...
> 폐기조건: `/context` 값을 스크립트로 읽을 수 있게 되면 이 추정식을 그것으로 교체한다.
| MEMORY.md 항목 수 | `grep -c '^[-*] '` (목록 항목) | < 30 |

> ⚠️ **측정식 정정 (M-4, 2026-08-22)**: 종전 `grep "^## "` 는 현재 MEMORY.md 형식에서
> **항상 0** 이었다 — 이 파일은 `# Memory Index` 한 줄 아래 `- [제목](파일) — 요약`
> 목록으로 쓰므로 `##` 헤딩이 존재하지 않는다. 0 을 "항목 없음"으로 읽으면 이 지표가
> 영원히 만점으로 보인다. 재현: `grep -c '^## ' MEMORY.md` → 0 vs `grep -c '^[-*] '` → 실제값
> 폐기조건: MEMORY.md 가 헤딩 기반 형식으로 바뀌면 측정식을 되돌린다.
