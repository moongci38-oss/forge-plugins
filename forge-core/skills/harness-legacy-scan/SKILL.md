...
**텔레메트리 신호(참고, 대체 아님)**: `$HOME/.claude.json`의 네이티브 skillUsage/pluginUsage 카운터(`shared/scripts/skill-telemetry.py`)를 스캔 전 1회 조회해 Lens 3(스킬 품질)·Lens 4(제품 중복)에 주입 — 기존 3경로 배선측정(skill-usage.py)의 보강 신호일 뿐이며 **이 운영자 이 머신 로컬 집계**라 팀 전체·타 환경 사용량은 반영하지 않는다. usage=0을 DELETE 근거로 쓰지 않는다 (2026-08-03 배선, fail-open).
