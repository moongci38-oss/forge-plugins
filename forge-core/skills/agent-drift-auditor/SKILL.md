...
description: "⛔ DEPRECATED(2026-08-11) — 유지보수 중단. 파이프라인의 agent drift 4종(삭제 agent 호출·중간 산출물 잔존·외부발송 미승인·모델 핀 누락/구버전)을 감사한다. PR 머지 전 하네스 무결성을 점검할 때 사용한다."
---
...
>
> **부분 대체**: `/system-audit` — 넓은 중복·드리프트 감사가 일부 겹칩니다.
>
> ⚠️ **흡수된 것이 아닙니다.** 이 스킬의 아래 4개 검사에 대응하는 항목은 `/system-audit` 에
> 없습니다: ①삭제된 에이전트 호출 ②중간 산출물 잔여 ③미승인 외부 발신 ④모델 pin 부재·구버전.
> 그 4개가 필요하면 수동으로 확인하십시오.
>
> **모델이 자동으로 고르지 않습니다**(`disable-model-invocation`). 유지보수가 끝난 스킬을
> 모델이 계속 집어 쓰면 경고가 무의미해집니다 — `/agent-drift-auditor` 로 **명시 호출하면 그대로 동작**합니다.
>
