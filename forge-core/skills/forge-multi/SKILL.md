---
name: forge-multi
description: "Multi-worker 검수 — Claude(Opus 5) + Codex(GPT-6 Astra) 2벤더 교차, 가중 0.5/0.5. 트리거: /forge-multi, /cr-double, /cr-triple, plan/spec 저장 후 자동, plateau 3회 자동승격."
---
...
```bash
# 어느 쪽을 불러도 같다 — Claude(Opus 5) + Codex(GPT-6 Astra) 2벤더 교차
...
| 레그 | Worker | 가중 |
|------|--------|------|
| Claude | `advisor-strategist` 틀 (**Opus 5** — `--fable` 명시 시 Fable 5.1 · 등급 `claudeModel` 이 오면 그 값) | **0.5** |
| Codex | `mcp__codex__codex` (GPT-6 Astra — advisor 전용의 명시적 예외 · `--sol`/`--terra`/`--luna`·`frontier:false` 시 하향) | **0.5** |

⚠️ **2026-09-17 Human 지시로 Claude 레그 기본이 Fable 5.1 → Opus 5 로 바뀌었다**(구 표기 "Claude(Fable 5.1)" 폐기).
근거: 최고급 모델(Fable 5.1·GPT-6 Astra)은 **advisor 전용**이고 구현·지적 수정은 Opus 5 + gpt-5.6-sol 이다. 검수 레그는 Opus 5 + **Codex Astra**(명시적 예외).
실측 배경: PR #579 r4 에서 이 레그가 **Sonnet 으로 떨어져** 2라운드 연속 0건을 냈다(막는 결함은 전부 Codex 가 찾았다).
`--fable` 을 명시하면 종전대로 Fable 5.1 로 돈다. 기계 검증 = `.claude/skills/forge-multi/tests/model-defaults.test.mjs`.
⚠️ 레포의 다른 문서(README·CLAUDE.md·`dev/global-rules/model-routing.md` 등)에는 아직 "Claude=Fable 5.1" 표기가 남아 있다 — 별건 문서 스윕 대상이다.
...
1. **Workflow 반환값** — 사람이 읽는 검수 결과(`verdict`/`combined`/`issues[]`/`degraded`/
   ...
   - ⚠️ 이 두 키를 **읽는 기계 소비처는 아직 0개**다(2026-09-03 실측 — 보고·조사용). 문을 잠그는
     것은 `verdict` 뿐이다.
...
```bash
bash ~/.claude/scripts/cache-stats-logger.sh forge-multi "$MODEL" "$CACHE_READ" "$CACHE_CREATION" "$RAW_INPUT" cr-review
```
...
// 호출 규약(단일 — 2026-09-15 v2 C-2): 매 호출 SSoT 를 cat 한다(scriptPath 세션 사본·~/.claude 미러 금지 — 낡은 사본은 엔진이 stale_engine 으로 거부).
...
// machineChecks: {ran:[syntax|md-dup|rule-meta|claude-lines|pr-fields|self-count|mutation ...], summary} (2.4.0) — /forge-pr §3 (b) 가 실제로 돌린 축만. 없으면 레그가 전 축을 본다.
//   2.5.0: 기계 축 mutation-marker · 실행형 축 tests|lint|wiring|secrets|repro 도 받는다(sec-paths·boundary 플래그는 제외 목록에 안 올린다) — 레그는 이 일을 **직접 하지 않고** 첨부에 없는 축을 '미실행' 1건으로만 적는다.
// 위험 등급(2.5.0, review-diet — /forge-pr §3 (c) 가 cr-risk-tier.sh 출력으로 싣는다. 미지정 = Opus 5 + Astra 병렬 2레그 — 2.7.0, 2.6.0 의 Opus+sol 을 되돌림 · 구 Fable+Astra · light 단일 레그는 sol):
//   crTier: skip → 레그 0 · INVALID_INPUT code=tier_skip(skip-merge 경로로 가라) · light → legs:1 이면 작성 반대편 벤더 1레그(정족수 1)
//           · full-general|full-gate → 2레그 **순차 단락**(Claude 먼저, CRITICAL/HIGH 면 Codex 생략 → short_circuited:true · r1 fix_and_rereview)
//   claudeModel: fable|opus|sonnet|claude-<그 셋>-… (별칭으로 접는다) · Claude effort = full-gate xhigh / 그 밖 등급 high
//   legs: 1|2 · legVendor?: claude|codex(light 단일 레그 강제 — 없으면 authorVendor=gpt→claude, 그 밖→codex) · crShortCircuit:false = 단락 끔
//   payload: tier · expected_legs · short_circuited · claude_model · claude_effort · codex_effort(원장 light 계수·등급 바인딩이 읽는다)
//   ⚠️ 순차 단락은 clean 런의 지연을 레그 1개 시간만큼 늘린다(병렬 → 순차).
Workflow({
...
- 명령: `~/forge/.claude/commands/forge-multi.md`
...
⑩ **레그를 손으로 조립했을 때도 `SINGLE_EXECUTOR_WARN_CAP` 이 그대로 적용된다** (2026-09-02 신설).
   `Workflow` 를 못 써서 레그를 수동으로 돌린 경우, **실제 실행체가 1개면 combined 가 80 을 넘어도
   verdict 는 WARN 이다.**
   ⚠️ **WARN 을 막는 것은 기계 게이트가 아니라 산문 규약**이다 — `pr-verdict-gate.sh` 는 WARN 을
   `exit 0` 으로 통과시킨다. 실제로 자동 머지를 막는 것은 `forge-pr.md §Step 3`("WARN 은 자동
   머지하지 않는다")이고, 그건 사람·에이전트가 읽고 지키는 문장이다(2026-09-03 정정). 대체된 레그(`provenance.executed_by="claude"`)와
   출처 미선언 레그는 **눈으로 세지 않는다** — 세는 것은 "몇 개가 응답했나"가 아니라 "서로 다른
   모델이 몇 개나 봤나"다.
   ⚠️ 수동 경로는 `inconclusive_legs` 를 채우지 않아 **기계가 정족수를 못 센다** — 그래서 이 항은
   기계 게이트가 아니라 **사람이 지켜야 하는 규약**이다. 보고에 `distinct_executors=N` 을 반드시 적는다.
   ⚠️ **이건 보안 경계가 아니라 품질 게이트다.** 레그가 `executed_by` 를 거짓 신고하면 이 상한은
   무력하다 — 이 게이트는 **정직한 레그의 '대체 사실'을 잡는 것**이지 적대적 레그를 막는 것이 아니다.
   구조적 검증(레그 격리·서명)은 별건이다(harness-gaps 등록).
   왜: #457 이 레그 1개로 PASS 머지된 경로가 정확히 이것이었다. `workflow.js` 의 상한은 그 경로를
   지나가지 않는다(코드를 아예 안 거친다). 쉽게 말하면 **자동문에만 잠금을 달아 놓고 옆의 쪽문은
   열어 둔 상태**였다.
