...
`$HOME/.claude/checkpoints/YYYY-MM-DD-HH-MM.md` 1개 파일(진행 중 태스크/완료/다음 스텝/블로커/열린 파일/Git 상태, 20~40줄) + 사용자 안내 메시지. handover·learnings·INDEX 갱신은 하지 않는다.
...
- `list` 서브커맨드: `/checkpoint list` 입력 시 `ls -lt $HOME/.claude/checkpoints/` 출력 (저장 안 함)
...
```
체크포인트 저장: $HOME/.claude/checkpoints/YYYY-MM-DD-HH-MM.md
...
1. `$HOME/.claude/checkpoints/` 에서 파일명 내림차순 정렬 후 최신 파일 read (파일명 정렬 기준: `YYYY-MM-DD-HH-MM.md` 사전순 역순)
2. 체크포인트 기준으로 상태 복원
