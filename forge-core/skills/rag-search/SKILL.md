...
⚠️ **문서 검색 전용 — 코드 심볼 검색이 아니다.** 함수·클래스·변수 같은 **코드 심볼**을 찾을 때는 `Grep`(정확 일치)이나 GitNexus(호출관계·영향범위)를 쓴다. 이 스킬은 `forge-outputs/` 산문 문서를 **의미**로 찾는 도구라, 심볼명처럼 의미가 얕은 토큰에는 유사도가 잘 안 걸린다(사전으로 전화번호를 찾는 격).
