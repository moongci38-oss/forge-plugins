...
description: >
  ...
  SKIP for: full Figma design-token sync, code extraction from Figma, or non-whitelist bulk export.
...
Figma node-id 화이트리스트(CSV 또는 매핑표)와 함께 "화면정의"/"목업 갤러리"/"Figma 프레임 추출" 요청 시 발동. 전체 디자인 토큰 동기화는 이 스킬의 범위 밖(`figma-design-sync` 는 2026-08-11 미사용으로 제거).
