...
> ℹ️ **위임 후 규모 판정은 `/prd` 레인이 소유한다** — 거기서 `/autoplan` 3관점 리뷰가 MANDATORY 로
> 걸린다(BLOCK≥1 → 수정·재호출 3회). 이 디스패처는 규모를 판정하지 않는다(정보 포인터 1줄, 동작 변경 0
> — `forge-design` 은 "기존 동작 100% 보존, 디스패처일 뿐"이 계약이다).
