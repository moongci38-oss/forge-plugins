...
> **경로 표기 규약**: 아래 프로젝트 경로는 **WSL 형 하나로만** 적는다. Windows 세션에서는
> `/mnt/<드라이브>/…` 를 `<드라이브>:\…` 로 바꿔 읽는다(예: `/mnt/x/a/b` → `X:\a\b`).
>
> 같은 경로를 Windows·WSL **두 표기로 병기하지 않는다** — 이 문서는 PUBLIC 인 `forge-plugins`
> 로 배포되면서 사설 경로가 환경변수로 치환되는데, 두 표기가 **같은 변수로 collapse** 돼
> `` `${X}\a\` (WSL: `${X}/a/`) `` 같은 자기중복이 남는다(2026-08-06 실측 후 정리).
...
## 바둑이 (Baduggi) — `${BOARDGAMES_ROOT}/baduggi/` — 실측 2026-07-21
...
## 맞고 (MatGo) — `${BOARDGAMES_ROOT}/matgo/` — 실측 2026-07-21
...
## GodBlade — `${GODBLADE_ROOT}/` — 실측 2026-07-18
