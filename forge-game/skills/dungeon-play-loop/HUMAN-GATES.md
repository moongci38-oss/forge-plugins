...
# dungeon-play-loop — Human Gates & Budget
...
| G3 | 고위험 서버 변경 | 매출 크리티컬 경로(ProcessNewGachaPattern 리팩터)·DB 마이그레이션·프로토콜 변경 **착수 전** | 루프 소유자 |
| G4 | 파괴적 액션 | 파일 삭제·덮어쓰기·외부 publish 전 | 루프 소유자 |

G1, G2는 삭제 불가.

### Gate 처리 절차

1. 루프가 `${GODBLADE_ROOT}/loops/dungeon-play-loop/STATE.md`에 gate-request 기록
2. Human이 명시적 승인
3. 루프가 승인 확인 후 계속
...
| 항목 | 상한 | 초과 시 |
|------|------|--------|
| Max iterations | 12 | 즉시 중단 |
| call-budget | 2000 | hook WARN → Human 판단 |
| **wall-clock 상한** | 6시간 | 즉시 중단 |
