...
Phase-hard-gate 순서:
  ...
  2. Spec 작성
     ↓
  3. Codex 2차 게이트 (자동, blocking)
     - codex-review --stage spec 호출
     - FAIL → Spec 재작성 후 재통과 필수
     ↓
  4. [STOP] Human 승인
     ↓
...
**Phase 0.7 — 가정 사항 표면화**
- 요구사항에서 암묵적 가정 추출: DB 스키마, 인증 방식, 외부 API 의존
...
- Spec 작성 완료 후 PRD→FR→AC(acceptance_predicate)→디자인 아티팩트 체인 검증:
  ...
  2. 각 FR이 `acceptance_predicate` 보유 여부 (untestable FR=0 목표)
...
**감지 조건**: 사용자 요청 또는 DISCOVERY.md에 `LLM`, `AI`, `embedding`, `vector`, `RAG`, `model` 키워드 포함.
